...
if [ "$DO_AGENTS" = 1 ]; then
  ...
  BLOCK="$BEGIN
## $SLUG
- Use `bash skills/cron-automation/scripts/cron-helper.sh --lint <jobs.json>` before deploying a cron, `--audit` to find crons failing at runtime, and `--tz HH:MM wib` to get the UTC cron field.
$END"
  touch "$AGENTS"
